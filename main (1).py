...
from automation_rule import path_fun
...
def lambda_handler(event, context):
    ...
    logging.info(
        "Hit Time: %s, Request Received at: %s", hit_time_formatted, hit_time_formatted
    )
    ...
    logging.info(
        "Request Completed: %s, Time Taken: %s seconds",
        request_completed_time_formatted,
        time_taken,
    )

    # Record the end time of the function
    end_time = time.time()
    performance_matrix["end_time"] = "{}.{:03d}".format(
        time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_time)),
        int((end_time % 1) * 1000),
    )
    performance_matrix["execution_time"] = f"{end_time - start_time:.4f}"
    logging.info(
        "Request processed at %s.%03d",
        time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_time)),
        int((end_time % 1) * 1000),
    )
